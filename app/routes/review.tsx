...
import { cleaningsApi, qrApi } from '../shared/api'
...
const SuccessCleaning = () => (
  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center', gap: 24 }}>
    ...
    <div>
      <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: -0.6, marginBottom: 8, color: T.text }}>Готово</div>
      <div style={{ fontSize: 16, color: T.textMute, lineHeight: 1.4 }}>
...
const CleaningCapture = observer(function CleaningCapture({
  entranceId,
  floor,
  cleanerId,
}: {
  ...
  const submit = async () => {
    ...
    setError('')
    try {
      await cleaningsApi.submit({ cleanerId, entranceId, floor, photo })
      setDone(true)
    } catch (e: any) {
  ...
  if (done) return <SuccessCleaning />

  return (
    <div style={{ flex: 1, padding: '20px 20px 0', display: 'flex', flexDirection: 'column' }}>
      {/* Badge */}
      <div style={{ marginBottom: 24, display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 20, background: T.goodSoft, color: T.good, fontSize: 12, fontWeight: 700, fontFamily: FONT, alignSelf: 'flex-start' }}>
      ...
      <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: -0.6, lineHeight: 1.15, marginBottom: 8, color: T.text }}>
        Зафиксировать<br />уборку
      </div>
      <div style={{ fontSize: 15, color: T.textMute, marginBottom: 28, fontFamily: FONT }}>
        Этаж {floor}
      </div>
      ...
      {error && (
        <div style={{ marginTop: 12, fontSize: 13, color: T.bad, fontFamily: FONT }}>{error}</div>
      )}
      ...
      >
        {submitting ? <Spinner size={18} color="#373C46" /> : 'Отправить'}
      </Button>
    </div>
  )
})
...
const ReviewForm = observer(({ store, entranceId, floor }: { store: ReviewFormStore; entranceId: string; floor: number }) => {
  return (
    <div style={{ flex: 1, padding: '20px 20px 0', display: 'flex', flexDirection: 'column' }}>
      <div style={{ fontSize: 13, color: T.textDim, fontWeight: 500, letterSpacing: 0.1, marginBottom: 24, fontFamily: FONT }}>
        Подъезд · этаж {floor}
      </div>

      <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: -0.6, lineHeight: 1.15, marginBottom: 28, color: T.text }}>
      ...
      {store.rating && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, animation: 'slideDown 240ms cubic-bezier(.2,.7,.3,1)' }}>
          <textarea
            value={store.comment}
            ...
            style={{
              ...
              padding: '12px 14px', fontSize: 15, color: T.text, lineHeight: 1.4,
      ...
      {store.error && (
        <div style={{ marginTop: 12, fontSize: 13, color: T.bad, fontFamily: FONT }}>{store.error}</div>
      )}
...
export default observer(function ReviewPage() {
  ...
  const { auth, reviewForm } = useStore()
  ...
  useEffect(() => {
    ...
    qrApi.resolve(token)
      ...
      .catch(() => { if (!cancelled) setResolveError('QR-код не найден или устарел.') })
  ...
  const isValid = Boolean(entranceId) && !isNaN(floorNum) && floorNum > 0 && floorNum <= 100

  const isCleaner = auth.hasHydrated && auth.isAuthenticated && auth.role === 'cleaner'
  ...
  return (
    <div style={{ minHeight: '100dvh', maxWidth: 480, margin: '0 auto', background: T.bg, fontFamily: FONT, color: T.text, display: 'flex', flexDirection: 'column' }}>
      {isCleaner ? (
        <CleaningCapture entranceId={entranceId} floor={floorNum} cleanerId={auth.userId} />
      ) : reviewForm.isSubmitted ? (
        <SuccessReview />
      ) : (
